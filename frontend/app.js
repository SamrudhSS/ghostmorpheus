...
function calculateClientRisk(mId) {
    // DISABLED: We now rely entirely on AI Diagnostics from the simulation server
    // to avoid dashboard clutter and redundant alerts.
...
/**
 * Polls the simulation server for AI-generated alerts and syncs them to the local alerts array.
 */
...
async function startAlertPolling() {
    try {
        ...
        simAlerts.forEach(a => {
            if (!polledAlertIds.has(a.id)) {
                // Determine if it's an AI diagnostic or a system warning
                const isAI = a.reason.includes('AI_DIAGNOSTIC');
                const title = isAI ? 'PREDICTIVE_FAULT' : 'SYSTEM_ALERT';
                const cleanReason = a.reason.replace(/AI_DIAGNOSTIC\[Risk [0-9.]*\]: /, '');
                ...
                const newAlert = {
                    ...
                    type: isAI ? 'critical' : 'warning'
                };
                
                alerts.unshift(newAlert);
                polledAlertIds.add(a.id);
                updated = true;
            }
        });

        // Keep local buffer manageable
        if (alerts.length > 10) {
        ...
        if (updated) {
            ...
            if (selectedMachine === latest.machineId) {
                const diagNarrative = document.getElementById('ai-narrative');
                if (diagNarrative) {
                    diagNarrative.innerHTML = `<span class="text-error font-bold">[!] ${latest.timestamp}</span>: ${latest.message}`;
                }
            }
        }
    } catch (err) {
        console.warn('Alert polling failed. Simulation server might be offline.');
    }
}
...
async function startMaintenancePolling() {
    try {
        ...
        const feedOverview = document.getElementById('maintenance-schedule-feed-overview');
        const feedMain = document.getElementById('maintenance-schedule-feed');
        ...
        const renderFeed = (container) => {
            ...
            if (bookings.length === 0) {
                container.innerHTML = '<div class="text-[10px] font-mono text-outline uppercase p-4 text-center">No repairs scheduled.</div>';
            } else {
